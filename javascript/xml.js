...
require(['greys'], function (greys) {
    ...
    function getMethodType(name) {
        if(name == "SendandReceive"){
            return 1;
        }
        if(name == "PackString"){
            return 2;
    ...
    function getExternalMethodBeforeContent(advice,content){
        return "\n\n##################################Begin##################################:[{classname} {methodname}]\n\n".format({'classname': advice.clazz.name,
            'methodname': advice.method.name});
    }

    function getExternalMethodFinishContent(advice, context){
        var content ="\n\n##################################End##################################"
        content +=getCommonContent(advice,context)
        content += "params length="+advice.params.length+";"
    ...
    function getCommonContent(advice, context){
        return "{0} : cost={1}ms;".format(prefix(advice), context.get(getCostTimeKeyPrefix(advice)+"cost_after")-context.get(getCostTimeKeyPrefix(advice)+"cost_before"));
    }

    function getExceptionInfo(advice){
        var content = ""
        // 拼装异常信息
    ...
    function getCostTimeKeyPrefix(advice){
        return advice.method.name + advice.params.length;
    }
    ...
    greys.watching({
        returning: function (output, advice, context) {
            context.put(getCostTimeKeyPrefix(advice)+"cost_after",new Date())
